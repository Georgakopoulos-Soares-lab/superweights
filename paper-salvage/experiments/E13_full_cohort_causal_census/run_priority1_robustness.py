...
`results/e7_legacy_reanalysis.json`, which has no activation-ratio field at all -- those
...
Existing artifacts only -- no new model runs. Sources:
  ...
  - results/e7_phase1_detection_qwen25.json         (candidate.ratio, Qwen2.5-7B)
  ...
  - results/e7_phase1_detection_genomeocean.json    (candidate.ratio, GenomeOcean-4B)
...
def main() -> None:
    ...
    report["twentytwo_model_sensitivity_note"] = (
        ...
        "candidates came from results/e7_legacy_reanalysis.json (published/structural "
