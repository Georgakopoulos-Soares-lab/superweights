...
under `paper-salvage/experiments/` and `paper-salvage/src/`, not under the top-level
...
the ratio≥5.0 detector this task must reuse). `paper-salvage/CLAUDE.md` is the binding
...
runs until `python paper-salvage/src/prereg_lock.py verify <file>` prints `OK` for its
...
Lock mechanism: `paper-salvage/src/prereg_lock.py` — `lock <file>` hashes+appends to the
append-only ledger `paper-salvage/docs/prereg/LOCKS.jsonl`; `verify --all` / `verify <file>`
...
Protocol files live in **`paper-salvage/docs/prereg/`**, named `PREREG_<name>.md`
...
**Plan**: write `paper-salvage/docs/prereg/PREREG_E11_scale_ladder.md` and
`PREREG_E12_generator_degradation_control.md`, following this exact section skeleton, then
`python paper-salvage/src/prereg_lock.py lock <file>` each before any measurement — matching
...
3. Confirm `paper-salvage/docs/PAPER_OUTLINE.md` and `NEW_DIRECTION_EVIDENCE_AUDIT.md` (both
   flagged as required reading in `paper-salvage/CLAUDE.md` before writing anything
