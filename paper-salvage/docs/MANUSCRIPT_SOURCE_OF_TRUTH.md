...
figure-provenance work (`paper-salvage/figures/FIGURE_PLAN_v3.md`, `FIGURE_PROVENANCE.md`,
`FIGURE_CAPTIONS_v3.md`, and the exact JSON values saved in
`paper-salvage/figures/source_data/*.json`, each of which is itself loaded live from the raw
...
This is confirmed as the repo's live thesis: `paper-salvage/CLAUDE.md`'s frozen thesis section
and `paper-salvage/docs/DECISIONS.md` D-016 retire the older "predictable-routing" thesis in
...
- `paper-salvage/CLAUDE.md`'s frozen thesis statement (v2, 2026-08-13) explicitly retires this
...
- `paper-salvage/docs/PAPER_OUTLINE.md` (v2, 2026-08-13) is organized around a *different*,
...
visual encoding: `paper-salvage/figures/_paper_encoding.py` (marker shape = architecture,
color = domain). **The latest figure-generation scripts (`paper-salvage/figures/fig1_structural.py`
...
renders. Full per-panel JSON-key-level provenance: `paper-salvage/figures/FIGURE_PROVENANCE.md`.
...
**Retired diagonal approximation** (`paper-salvage/src/uk_frobenius.py`), weights-only, no
forward pass:
```
...
### 6.1 Preregistration versions (confirmed from `paper-salvage/docs/prereg/LOCKS.jsonl` and experiment RESULTS.md files read this session)
...
record: `paper-salvage/docs/MISSING_COLLEAGUE_ARTIFACTS.md`): GENERator sequence-quality/
...
**Ref [11] — flagged unresolved, per `paper-salvage/docs/REFERENCE_AUDIT.md`** (read in full):
...
| Intended venue | Not stated inside `paper/main.tex` itself. Target list recorded separately: "Positioning: Mechanistic-interpretability spine, computational-biology venue. Targets unchanged: **Genome Biology, Nature Methods, NAR Genomics & Bioinformatics, Patterns.**" | `paper-salvage/docs/PAPER_OUTLINE.md:25-26` |
