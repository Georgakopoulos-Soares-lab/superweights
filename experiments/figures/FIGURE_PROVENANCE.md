...
| B | Exact `q1` per model | `results/E7/e7_legacy_reanalysis.json` (Mistral/Llama/GENERator EUK/OLMo/DNABERT-2/NTv3); `results/E7/e7_phase1_detection_qwen25.json` (Qwen2.5); `results/E7/e7_phase1_detection_genomeocean.json` (GenomeOcean); `results/E7/e7_phi3_spectral.json` (Phi-3, model-level median); `results/e8_detection_{mosaicbert,modernbert}.json` (encoders); `results/E7/e7_phase1_detection_evo2.json` (Evo2, explicit null) | `.q1` / `.spectral.q1` / `.model_level.q1_median` | none | q1 (exact spectral concentration) organizes more by architecture than domain, descriptively | q1 is NOT a magnitude or causal-importance axis (enforced by never plotting it against effect size in this or any other figure) |
...
| Panel | Plotted variable | Artifact | Keys | Transformation | Claim supported | Caveat |
|---|---|---|---|---|---|---|
| A | GENERator EUK row 2371 exact q1 | `results/E7/e7_legacy_reanalysis.json` | `["GENERator EUK"].q1` | none | Minimal structural callout, cross-referencing Fig. 1B | Deliberately minimal — no activation-lifecycle artifact exists in this repo for GENERator |
...
|---|---|---|---|
| a | Phi-3's 6 individual rows' exact q1 | `results/E7/e7_phi3_spectral.json` | `.rows[].{layer,row,q1}`, `.model_level.q1_median` |
| b | All 5 decoders' same-layer control-row causal effects | `results/E10/e10_decoder_concentration.json` | `.models.{*}.controls[].relative_pct_change` |
...
even labeled as retired. `results/gue/gue_multiseed_ntv3_splice.json` remains the artifact of
