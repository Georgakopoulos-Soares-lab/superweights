...
EPSILONS = [0.5, 1.0]
RESULTS_DIR = ROOT / "results" / "experiments" / "E13"
...
# Old locked numbers (results/experiments/E10/e10_decoder_spectrum_*.json) for the Stage-0 comparison table.
...
def run_modernbert(with_controls: bool, local_files_only: bool = False) -> dict:
    ...
    return dict(model="modernbert-base", repo=spec["repo"], resolved_revision=resolved,
                ...
                candidate_layer=layer, candidate_row=row,
                old_reference="results/experiments/E10/e10_encoder_fit_results.json (key 'modernbert')",
                new_control_rows=crows, control_layer=layer,
                conditions=results)
