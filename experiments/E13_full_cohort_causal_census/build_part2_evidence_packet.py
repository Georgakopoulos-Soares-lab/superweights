...
ROOT = HERE.parents[1]
E13 = ROOT / "results" / "experiments" / "E13"
AUDIT2 = ROOT / "audit" / "round2"
FIGDIR = ROOT / "experiments" / "figures" / "main"
...
def main() -> None:
    ...
    meta = {r["model"]: r for r in read_csv(ROOT / "results" / "experiments" / "E11" / "scale_ladder_backfilled.csv")}
...
def write_packet(rows: list[dict], manifest: dict, stats: dict) -> None:
    ...
    lines += ["","These are descriptive only. Objectives, architectures, domains, tokenizers, and panel composition are confounded; no architecture/domain determination claim is licensed.","",
              ...
              "## Primary provenance paths","","- `results/experiments/E13/candidate_manifest.json`","- `results/experiments/E11/scale_ladder_backfilled.csv`","- `results/experiments/E13/raw/*.json`","- `results/experiments/E13/part1_22_candidate_effects.csv`","- `results/experiments/E13/part1_22_control_effects.csv`","- `results/experiments/E13/part1_22_cohort_summary.csv`","- `results/experiments/E13/part1_22_structure_function_correlations.json`","- `experiments/E13_full_cohort_causal_census/run_singleton_census.py`","- `experiments/E13_full_cohort_causal_census/write_part1_22_report.py`","- `experiments/E10_nlp_architecture_causal/e10_lib.py`","- `experiments/E9_mechanistic_tomography/tomography_lib.py`","- `experiments/E12_generator_degradation_control/e12_lib.py`","- `results/experiments/E13_dnabert2_reproducibility/DNABERT2_EXECUTION_PATH_DIAGNOSTIC.md`",""]
    OUT_MD.write_text("\n".join(lines))
