...
OUT_DIR = RESULTS/"experiments"/"E13" / "priority1_robustness"
...
def load_activation_ratios() -> dict[str, float]:
    ...
    with (RESULTS/"experiments"/"E11" / "scale_ladder_backfilled.csv").open() as f:
...
def load_candidate_effects() -> dict[str, dict[float, dict]]:
    ...
    with (RESULTS/"experiments"/"E13" / "part1_22_candidate_effects.csv").open() as f:
        for row in csv.DictReader(f):
...
def load_random_control_gap() -> dict[str, dict[float, float]]:
    ...
    with (RESULTS/"experiments"/"E13" / "part1_22_cohort_summary.csv").open() as f:
        for row in csv.DictReader(f):
            m = row["model"]
            out[m] = {
