...
E5–E8 arc.** It diverged in mid-June, before `manuscript/` existed in any form (see §2).
...
**`manuscript/` does not exist on COLLEAGUE_BRANCH, and did not exist at the merge-base.**
Verified: `git ls-tree origin/mechanism-and-negative-results -- manuscript` and the same at
...
`manuscript/experiments/E5_dimensionality/` through `E8_encoder_decoder/`, the prereg
locks, `CLAIMS_LEDGER.md`, `PROJECT_STATUS.md`, or anything else under `manuscript/`.
...
| Nothing in E5/E6/E7/E8 | Colleague branch cannot touch these (§1 — `manuscript/` does not exist on it) | **Unaffected, structurally** |
...
- **`README.md`** — both branches edited it since the merge-base (colleague +138/−X lines,
  ...
  manuscript's later thesis pivot (Frobenius/dimensionality → exact operator geometry) or
...
- Nothing on this branch would overwrite or stale-ify current E5–E8 conclusions, because
  nothing on this branch touches `manuscript/` at all (§1). There is no file here that
