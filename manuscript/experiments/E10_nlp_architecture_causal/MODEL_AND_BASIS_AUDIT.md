...
(`/work/11034/atzanakak/glm_super_weight/genomic-super-weights/`); `manuscript/results/`
and the top-level `results/` are two different, both-real directories — raw JSON outputs
referenced below live in the top-level `results/`, not inside `manuscript/`.
...
- **Checkpoint / HF revision:** `huggyllama/llama-7b`. **No pinned revision recorded anywhere**
  — explicitly disclosed as a gap (`manuscript/experiments/E5_dimensionality/GATE0_RESULTS.md`,
...
- **Architecture / module path:** `model.model.layers[layer].mlp.{gate_proj, up_proj,
  ...
  `manuscript/src/uk_frobenius.py`. The E1 script fetches the same three tensor names
  directly by string (`manuscript/experiments/E1_nlp_validation/run_e1_retrospective.py`,
  `names = {"gate": "model.layers.{layer}.mlp.gate_proj.weight", ...}`).
- **Established high-gain row:** **L2 / row 3968** (output row `k`), scalar `i=7003`
  (`manuscript/experiments/E1_nlp_validation/RESULTS.md`, "Level 1 — row recovery" table;
  also `manuscript/experiments/E7_exact_dimensionality/RESULTS.md`, legacy discovery panel
  table: "Llama-7B | NLP | L2/r3968"). Sourced originally from Yu et al. (2024, arXiv:2411.07191)
  Table 2, read from `manuscript/docs/superweight_paper.txt`
  (`run_e1_retrospective.py` docstring/comment, "for Llama-7B ... layers[2].mlp.down_proj.weight[3968, 7003]").
- **U_k / Ux artifact:** `manuscript/src/uk_frobenius.py` (`uk_frobenius()` /
...
- **q1 / PR_spec:** **q1 = 0.9888**, `PR_spec = 1.0227`
  (`manuscript/experiments/E7_exact_dimensionality/RESULTS.md`, "Legacy discovery panel
...
- **Single row vs. SET:** **Single canonical row only.** Explicitly stated:
  ...
  independent second row exists to select." (`manuscript/experiments/E6_cross_geometry/CONFIRMATION_PANEL.md`,
...
- **Checkpoint / HF revision:** `mistralai/Mistral-7B-v0.1`. **No pinned revision recorded**
  (`manuscript/experiments/E5_dimensionality/GATE0_RESULTS.md`, line 144).
- **Architecture / module path:** `model.model.layers[layer].mlp.{gate_proj, up_proj,
  down_proj}` — identical `MistralMLP` SwiGLU layout to Llama, same `adapter_llama_swiglu`
  (`manuscript/src/uk_frobenius.py`, comment: "Llama / Mistral / GENERator: gate_proj,
  up_proj, down_proj").
- **Established high-gain row:** **L1 / row 2070**, scalar `i=7310`
  (`manuscript/experiments/E1_nlp_validation/RESULTS.md`, Level-1 table; also E7 legacy
...
- **Checkpoint / HF revision:** `allenai/OLMo-7B-0724-hf`. **No pinned revision recorded**
  (`manuscript/experiments/E6_cross_geometry/RESULTS.md`, provenance table: "`allenai/OLMo-7B-0724-hf`
...
- **Checkpoint / HF revision:** `microsoft/Phi-3-mini-4k-instruct`, resolved HF commit
  **`f39ac1d28e925b323eae81227eaba4464caced4e`**
  (`manuscript/experiments/E7_exact_dimensionality/PROVENANCE_ADDENDUM.md`, "Phi-3-mini-4k-instruct"
