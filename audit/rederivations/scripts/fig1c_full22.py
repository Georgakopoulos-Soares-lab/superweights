...
gap data (results/experiments/E11/scale_ladder_controls.csv, via fig1c_random_vs_topk_gaps.csv) with
the newly-computed 10-model batch-2 random-control q1s
(audit/rederivations/section2_random_control_q1_batch2.csv, same SeedSequence(42) convention)
and the full-22 topK-by-norm gaps (audit/rederivations/full22_topk_gap_confounds.csv).

Outputs:
  audit/rederivations/fig1c_random_vs_topk_gaps_full22.csv -- both gaps per model + cohort medians
  audit/rederivations/figures/fig1c_two_panel.png -- candidate-vs-random-controls (left),
...
OUT_CSV = ROOT / "audit/rederivations/fig1c_random_vs_topk_gaps_full22.csv"
OUT_FIG_DIR = ROOT / "audit/rederivations/figures"
...
batch2_ctrl = list(csv.DictReader(open(ROOT / "audit/rederivations/section2_random_control_q1_batch2.csv")))
...
full22 = list(csv.DictReader(open(ROOT / "audit/rederivations/full22_topk_gap_confounds.csv")))
