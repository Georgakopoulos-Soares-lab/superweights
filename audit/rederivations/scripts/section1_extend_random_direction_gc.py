...
Reuses the cached corpora (results/experiments/E12/raw/corpora.json) and reconstructs unit_dir with
...
Writes NEW checkpoint file (does not touch the original gen_records.jsonl):
  audit/rederivations/raw/gc_extension_records.jsonl  -- full per-prompt records (gc, entropy,
    self_nll, raw/filtered sequence, etc.) for every (c, seed, prompt) triple.
Also writes audit/rederivations/generator_random_direction_gc_extension.csv (aggregated).
"""
...
E12_DIR = ROOT / "experiments/frozen/E12_generator_degradation_control"
sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(E12_DIR))
import e12_lib as el  # noqa: E402

OUT_DIR = ROOT / "audit/rederivations/raw"
OUT_DIR.mkdir(parents=True, exist_ok=True)
GEN_CKPT = OUT_DIR / "gc_extension_records.jsonl"
SUMMARY_CSV = ROOT / "audit/rederivations/generator_random_direction_gc_extension.csv"
...
CORPUS_CACHE = ROOT / "results/experiments/E12/raw/corpora.json"
