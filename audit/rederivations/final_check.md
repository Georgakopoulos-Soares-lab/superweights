...
Reference documents: `audit/AUDIT_REPORT.md`, `audit/rederivations/AUDIT_ROUND2_REPORT.md`, and the
...
Compute both counts directly from `audit/rederivations/structural_vs_causal_gap.csv` as
...
All three need source-data CSVs alongside them, in `audit/rederivations/figures/` and
`experiments/figures/source_data/`. Match the existing figure style in
`experiments/figures/` (fonts, marker conventions, encoder/decoder shape coding, text/genomic
...
`audit/rederivations/structure_function_correlations.csv`, with the underpowered flag retained.
...
I want this tested rather than assumed. Confirm `results/experiments/E7/e7_phase1_detection_evo2.json` is tracked.
...
The thing I most need for writing. One CSV, `audit/rederivations/manuscript_numbers.csv`, listing every
