...
alpha-sweep and the five inert control rows. Produces audit/round2/generator_random_direction_full.csv
and two plots in audit/round2/figures/.
"""
...
dm_rows = list(csv.DictReader(open(ROOT / "results/E12/damage_matching.csv")))
rd = [r for r in dm_rows if r["label"] == "random_direction"][0]
grid_nll = {g["scale"]: g["damage"] for g in json.loads(rd["grid_json"])}

dose_rows = list(csv.DictReader(open(ROOT / "results/E12/dose_response_extended.csv")))
gc_existing = {}
for r in dose_rows:
...
damage_evals = [json.loads(l) for l in open(ROOT / "results/E12/raw/damage_evals.jsonl")]
