...
Sources: candidate q1 / PR_spec / ||U_k||_F / layer-relative Frobenius -- `audit/census_master.csv` (STORED). Layer median norm -- `audit/round2/topk_norm_controls.csv` col `layer_median_norm` (STORED, all 22 models). Mean random-control q1 and both gap columns -- `audit/round2/fig1c_random_vs_topk_gaps_full22.csv` (STORED, reused verbatim per task spec). Mean top-norm-control q1 -- RECOMPUTED as the average of the 5 `rank2_by_norm`..`rank6_by_norm` q1 values per model from `audit/round2/topk_norm_q1_summary.csv`.
