...
#SBATCH -o /work/10906/arisk/ls6/SuperWeights/results/general_superactivations/slurm.%j.out
#SBATCH -e /work/10906/arisk/ls6/SuperWeights/results/general_superactivations/slurm.%j.err
...
mkdir -p /work/10906/arisk/ls6/SuperWeights/results/general_superactivations || true
...
# Postprocess: produce two-track reports (general vs spiky) from the run outputs.
: "${RUN_TWO_TRACK_POSTPROCESS:=1}"
: "${TWO_TRACK_TOP_GENERAL_CHANNELS:=${TOP_CHANNELS}}"
: "${TWO_TRACK_TOP_SPIKY_CHANNELS:=${TOP_CHANNELS}}"
: "${TWO_TRACK_SPIKY_METRIC:=tail_ratio_qhigh_median}"
: "${TWO_TRACK_TOP_WINDOWS_PER_CHANNEL:=${TOP_WINDOWS_PER_CHANNEL}}"
: "${TWO_TRACK_CONTROL_SET:=random}"
: "${TWO_TRACK_WRITE_PLOTS:=0}"
...
if [[ "${RUN_TWO_TRACK_POSTPROCESS}" == "1" ]]; then
  post_cmd=(
    "$PY" -u /work/10906/arisk/ls6/SuperWeights/scripts/two_track_superweights_postprocess.py
    --run_dir "$OUT_DIR"
    --control_set "$TWO_TRACK_CONTROL_SET"
    --top_general_channels "$TWO_TRACK_TOP_GENERAL_CHANNELS"
    --top_spiky_channels "$TWO_TRACK_TOP_SPIKY_CHANNELS"
    --spiky_metric "$TWO_TRACK_SPIKY_METRIC"
    --top_windows_per_channel "$TWO_TRACK_TOP_WINDOWS_PER_CHANNEL"
  )
  if [[ "${TWO_TRACK_WRITE_PLOTS}" == "1" ]]; then
    post_cmd+=(--write_plots)
  fi
  echo "Running two-track postprocess: ${post_cmd[*]}" | tee "$OUT_DIR/two_track_cmd.txt"
  "${post_cmd[@]}" 2>&1 | tee "$OUT_DIR/two_track.log"
fi
