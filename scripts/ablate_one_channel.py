...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Ablate a single (layer, channel) and report ΔAUC/ΔSpearman(beta). "
            "Writes one parquet row per seed and a JSON summary."
        )
    )

    p.add_argument("--layer", required=True, type=str)
    ...
    p.add_argument(
        ...
        type=str,
        default="1337",
        help="Comma-separated seeds (e.g. '1337,2024,7'). If n_pos/n_neg are 'all', seeds will not change the set.",
    )

    p.add_argument("--out", type=str, default="results/ablation/ablate_one_channel.parquet")
    p.add_argument("--report_json", type=str, default="results/ablation/ablate_one_channel.report.json")
...
def _spearman_rankcorr(x: np.ndarray, y: np.ndarray) -> float:
    """Fast Spearman via rank transform + Pearson."""
...
def main() -> None:
    ...
    for i, seed in enumerate(seeds, start=1):
        logger.info(f"[{i}/{len(seeds)}] seed={seed} building run table")
        df_run, overlap_stats = _build_run_table(df_parquet, args.pos_ids, args.neg_ids, n_pos=n_pos, n_neg=n_neg, seed=seed)
        if overlap_any is None:
            overlap_any = overlap_stats
        ...
        logger.info(f"[{i}/{len(seeds)}] seed={seed} baseline scoring (n={len(df_run)})")
        ...
        logger.info(f"[{i}/{len(seeds)}] seed={seed} ablation {layer} ch={ch}")
    ...
    df_out = pd.DataFrame(rows).sort_values(["delta_auc"], ascending=False).reset_index(drop=True)
    write_parquet(df_out, out_path)
    logger.info(f"Wrote parquet: {out_path} (rows={len(df_out)})")

    delta_auc_vals = df_out["delta_auc"].to_numpy(dtype=np.float64)
    delta_spear_vals = df_out["delta_spearman_beta"].to_numpy(dtype=np.float64)
    ...
    report = {
        ...
        "outputs": {"parquet": str(out_path), "report_json": str(report_path)},
        "overlap": overlap_any,
        "summary": {
            "delta_auc_mean": float(np.nanmean(delta_auc_vals)),
            "delta_auc_std": float(np.nanstd(delta_auc_vals)),
            "delta_spearman_beta_mean": float(np.nanmean(delta_spear_vals)),
            "delta_spearman_beta_std": float(np.nanstd(delta_spear_vals)),
        },
    }
